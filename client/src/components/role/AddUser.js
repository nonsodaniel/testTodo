...
import { createUsers } from '../store/actions/authActions'
...
class AddUser extends Component {
    ...
    handleSubmit = (e) => {
        ...
        let firstname = regDetails.firstname, othernames = regDetails.othernames,
            email = regDetails.email, phone = regDetails.phone, school = regDetails.school,
            dob = regDetails.dob, image = regDetails.image, password = regDetails.password;


        let obj = { firstname, othernames, email, phone, image, password, school, dob };
        console.log("Objects", obj)
        this.props.createUsers(obj)
            ...
            <div className="main-content-container container-fluid px-4" style={{ background: "" }}>
                ...
                <div className="page-header row no-gutters py-4" style={{ height: "0px" }}>
                    <div className="col-12 col-sm-4 text-center text-sm-left mb-0">
                        <h6 className="page-title">User Registration</h6>
                    </div>
                </div><br />
...
const mapDispatchToProps = (dispatch) => {
    return {
        createUsers: (user) => dispatch(createUsers(user))
    }
}
